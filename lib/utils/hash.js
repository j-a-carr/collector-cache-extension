 ...
 * @returns {{ hashes: object, transformedFiles: string[] }|null} Hashes and transformed files list,
 *   or null if any source missing
 */
function computeHashes (worktree, sources, logger, componentName, key, compiledTransforms) {
  ...
  for (const source of sources) {
    ...
    if (compiledTransforms && compiledTransforms.length > 0) {
      const result = applyTransforms(source, content, compiledTransforms, logger)
      content = result.content
      if (result.transformed) {
        transformedFiles.push(source)
      }
    }
  ...
  return { hashes, transformedFiles }
}
