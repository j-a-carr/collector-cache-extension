...
module.exports.register = function ({ config = {} }) {
  ...
  // Note: Antora normalizes YAML keys like hash_transforms to camelCase (hashTransforms)
  const extHashTransforms = config.hashTransforms || config.hash_transforms
      ...
      for (const origin of origins) {
        ...
        for (const entry of entries) {
          ...
          try {
            ...
            const sourceHashes = computeHashes(
              worktree, resolvedSources, logger, componentName, key, compiledTransforms
            )
    ...
    for (const entry of cacheEntries) {
      try {
        ...
        if (!sourceHashes) {
          ...
          sourceHashes = computeHashes(
            worktree, resolvedSources, logger, entry.componentName, entry.key, entry.compiledTransforms
          )
          if (!sourceHashes) {
