...
// Import utilities
const { computeHashes, computeContentHash, computeOutputHash } = require('./utils/hash')
...
const { compileTransforms } = require('./utils/transform')
...
module.exports.register = function ({ config = {} }) {
  ...
  // Compile hash transforms from extension config (applies to all components)
  // Note: Antora normalizes YAML keys - hash_transforms becomes hashTransforms
  const extHashTransforms = config.hashTransforms || config.hashtransforms
  const compiledTransforms = extHashTransforms ? compileTransforms(extHashTransforms, logger) : null

  if (compiledTransforms && compiledTransforms.length > 0) {
    logger.info(`Compiled ${compiledTransforms.length} hash transform pattern(s)`)
  }
        ...
        if (!worktree) {
          ...
          try {
            ...
            if (origin.url) {
              ...
              // Use computed refname since origin.refname may be undefined
              const originWithRefname = { ...origin, refname }
              const worktreeDirName = generateWorktreeFolderName(originWithRefname, true)
              worktree = path.join(collectorCacheDir, worktreeDirName)
          ...
          for (const entry of entries) {
            ...
            // Determine effective transforms for this entry (global + per-entry)
            const entryHashTransforms = run.hashTransforms || run.hashtransforms
            const requestedMode = run.hashTransformsMode || run.hashtransformsmode || 'extend'

            let effectiveTransforms
            let actualTransformMode
            if (requestedMode === 'ignore') {
              effectiveTransforms = null
              actualTransformMode = 'ignore'
            } else if (requestedMode === 'replace') {
              effectiveTransforms = entryHashTransforms ? compileTransforms(entryHashTransforms, logger) : null
              actualTransformMode = 'replace'
            } else {
              // 'extend' mode (default)
              const compiledEntryTransforms = entryHashTransforms ? compileTransforms(entryHashTransforms, logger) : []
              effectiveTransforms = [...(compiledTransforms || []), ...compiledEntryTransforms]
              if (effectiveTransforms.length === 0) {
                effectiveTransforms = null
                actualTransformMode = null
              } else if (compiledEntryTransforms.length === 0) {
                actualTransformMode = 'global-only'
              } else {
                actualTransformMode = 'extend'
              }
            }
            ...
            cacheEntries.push({
              ...
              componentHashDir,
              key: run.key,
              command: run.command, // Store command for hash calculation
              ...
              contentHash: null,
              effectiveTransforms,
              transformMode: actualTransformMode,
              transformedFiles: [],
            })
          }
          continue
        ...
        for (const entry of entries) {
          ...
          try {
            ...
            if (!fs.existsSync(worktree)) {
              ...
              // Determine effective transforms for this entry
              const entryHashTransforms2 = run.hashTransforms || run.hashtransforms
              const requestedMode2 = run.hashTransformsMode || run.hashtransformsmode || 'extend'

              let effectiveTransforms2
              let actualTransformMode2
              if (requestedMode2 === 'ignore') {
                effectiveTransforms2 = null
                actualTransformMode2 = 'ignore'
              } else if (requestedMode2 === 'replace') {
                effectiveTransforms2 = entryHashTransforms2 ? compileTransforms(entryHashTransforms2, logger) : null
                actualTransformMode2 = 'replace'
              } else {
                const compiledEntryTransforms2 = entryHashTransforms2
                  ? compileTransforms(entryHashTransforms2, logger)
                  : []
                effectiveTransforms2 = [...(compiledTransforms || []), ...compiledEntryTransforms2]
                if (effectiveTransforms2.length === 0) {
                  effectiveTransforms2 = null
                  actualTransformMode2 = null
                } else if (compiledEntryTransforms2.length === 0) {
                  actualTransformMode2 = 'global-only'
                } else {
                  actualTransformMode2 = 'extend'
                }
              }
              ...
              cacheEntries.push({
                ...
                sourceHashes: null,
                contentHash: null,
                effectiveTransforms: effectiveTransforms2,
                transformMode: actualTransformMode2,
                transformedFiles: [],
              })
              continue
            }
            ...
            if (requestedMode === 'ignore') {
              ...
              // 'extend' mode (default): global transforms + entry transforms
              const compiledEntryTransforms = entryHashTransforms ? compileTransforms(entryHashTransforms, logger) : []
              effectiveTransforms = [...(compiledTransforms || []), ...compiledEntryTransforms]
              if (effectiveTransforms.length === 0) {
                effectiveTransforms = null
            ...
            // Compute source file hashes (with optional transforms)
            const hashResult = computeHashes(
              worktree, resolvedSources, logger, componentName, key, effectiveTransforms
            )
            ...
            const { hashes: sourceHashes, transformedFiles } = hashResult

            // Compute content hash from source hashes and command
            const contentHash = computeContentHash(sourceHashes, run.command)
            ...
            if (shouldSkip) {
              ...
              // Always restore cached outputs to worktree so collector can scan from there.
              // This is necessary because the collector extension deletes scan directories before
              // running commands (see @antora/collector-extension line 55). If we point scan to
              // our cache directory, the collector deletes it and subsequent runs get MISS.
              ...
              copyDirectory(cacheOutputPath, worktreeOutputPath, logger)
              logger.debug(`Restored cached outputs to worktree: ${worktreeOutputPath}`)

              // Also restore any additional files specified by restoreToWorktree patterns
              ...
              // Let collector scan from worktree (normal behavior).
              // The command is 'true' (no-op) since outputs are already restored from cache.
              if (scan) {
                origin.descriptor.ext.collector.push({
                  run: {
                    command: 'true', // No-op - outputs already restored from cache
                  },
                  scan, // Use original scan config (points to worktree)
                })
              }
            } else {
  ...
  this.on('beforePublish', async ({ playbook }) => {
    ...
    for (const entry of cacheEntries) {
      try {
        ...
        let transformedFiles = entry.transformedFiles || []
        ...
        if (!sourceHashes) {
          ...
          const hashResult = computeHashes(
            worktree, resolvedSources, logger, entry.componentName, entry.key, entry.effectiveTransforms
          )
          if (!hashResult) {
            logger.warn(`Source files still not found for ${entry.componentName}/${entry.key}`)
            continue
          }
          sourceHashes = hashResult.hashes
          transformedFiles = hashResult.transformedFiles
          contentHash = computeContentHash(sourceHashes, entry.command)
        }

        // Check if output directory exists before computing output hash
        ...
        // Compute hash of output files (v2: output-addressable storage)
        const outputHashResult = computeOutputHash(sourceOutputPath, logger)
        const outputHash = outputHashResult.hash

        // Create pointer file (v2 format with optional transforms info)
        const pointerDir = path.join(entry.componentHashDir, entry.key)
        fs.mkdirSync(pointerDir, { recursive: true })

        const pointer = {
          version: 2,
          outputDir: outputHash,
        ...
        // Add transforms debug info only if files were actually transformed
        if (transformedFiles.length > 0) {
          pointer.transforms = {
            applied: transformedFiles,
            mode: entry.transformMode || 'global-only',
            patterns: entry.effectiveTransforms?.map((t) => t.pattern) || [],
          }
        }
        ...
        // Check if outputs already cached (deduplication)
        const cachedOutputPath = path.join(playbook.dir, cacheDir, 'outputs', outputHash, entry.outputDir)

        if (checkOutputsExist(cachedOutputPath, logger)) {
          logger.info(
            `Deduplicated: ${entry.componentName}/${entry.key} → ${outputHash.substring(0, 12)}... (already cached)`
          )
        } else {
          copyDirectory(sourceOutputPath, cachedOutputPath, logger)
          logger.info(`Cached outputs for ${entry.componentName}/${entry.key} → ${outputHash.substring(0, 12)}...`)
        }
      } catch (error) {
